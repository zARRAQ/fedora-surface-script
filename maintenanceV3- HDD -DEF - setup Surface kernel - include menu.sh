...
# Remove strict error exit so menu stays alive on individual check failures
set +e
...
print_banner() {
    ...
    echo "║              Kernel | DNF | Filesystem | Services                    ║"
    echo "╚══════════════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
...
menu_filesystem() {
    header "4. Filesystem Maintenance"
    ...
    if [[ "$root_fs" == "btrfs" ]]; then
        ...
        if ask_yes_no "Run Btrfs defragmentation?"; then
            sudo btrfs filesystem defragment -r / || warning "Defrag completed with warnings"
        fi
...
# =============================================================================
# 10. Flatpak Maintenance (Fedora default)
# =============================================================================
...
menu_run_all() {
    ...
    warning "This will run all checks sequentially. Some may prompt for confirmation."
...
main_menu() {
    while true; do
        ...
        echo -e " ${GREEN} 4.${NC} Filesystem Maintenance"
        ...
        echo -e " ${MAGENTA}12.${NC} ${MAGENTA}→ Run ALL Maintenance Tasks${NC}"
